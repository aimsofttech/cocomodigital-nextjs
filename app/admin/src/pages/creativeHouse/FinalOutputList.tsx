...
export default function FinalOutputList() {
  ...
  const { data, loading, submitting, pagination, remove, setSearch, setPage, setFilterParams, fetchAll, setData } = useCrud(creativeHouseFinalOutputApi, true, itemId ? { creativeHouseItemId: itemId } : {});

  // Drag-and-drop rows to renumber displayOrder (shared hook).
  const handleReorder = useRowReorder({ api: creativeHouseFinalOutputApi, data, setData, pagination, fetchAll });
