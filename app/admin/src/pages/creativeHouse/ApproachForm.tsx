...
export default function ApproachForm({ onSuccess, onCancel, editId, lockedItemId }: Props = {}) {
  ...
  const onSubmit = async (formData: any) => {
    ...
    // title is required by the schema; mirror the heading into it.
    if (formData.heading) formData.title = formData.heading;
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input {...register('heading', { required: 'Required' })} className="form-input" placeholder="e.g. Concept & Storyboard" />
