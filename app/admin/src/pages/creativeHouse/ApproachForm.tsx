...
export default function ApproachForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div>
        <label className="form-label">Approach Title <span className="text-red-500">*</span></label>
        <input {...register('approach_title', { required: 'Required' })} className="form-input" />
        {errors.approach_title && <p className="form-error">{String(errors.approach_title.message)}</p>}
      </div>
      <SlugField register={register} watch={watch} setValue={setValue} isEdit={isEdit} />
      <div><label className="form-label">Description</label><textarea {...register('approach_description')} className="form-textarea" /></div>
      <ImageUpload name="approach_image" label="Approach Image" uploadType="image" folder="creative-house" value={watch('approach_image')} onChange={(url) => setValue('approach_image', url)} />
