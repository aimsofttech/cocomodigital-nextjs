...
export default function FinalOutputForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const onSubmit = async (formData: any) => {
    ...
    if (itemId) fd.append('creative_house_item_id', itemId);
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div><label className="form-label">Output Title</label><input {...register('output_title')} className="form-input" /></div>
        <div><label className="form-label">Video URL</label><input {...register('output_video_url')} className="form-input" placeholder="https://youtube.com/..." /></div>
      </div>
      <SlugField register={register} watch={watch} setValue={setValue} isEdit={isEdit} />
      <ImageUpload name="output_image" label="Output Image" uploadType="image" folder="creative-house" value={watch('output_image')} onChange={(url) => setValue('output_image', url)} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Edit Final Output' : 'Add Final Output'} breadcrumbs={[{ label: 'Final Output', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
