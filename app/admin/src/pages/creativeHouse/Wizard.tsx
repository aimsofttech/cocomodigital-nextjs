...
import { creativeHouseItemApi, creativeHouseApproachApi, creativeHouseFinalOutputApi } from '@/services/adminApi';
...
// navigation chips and the public creative page).
const SECTION_STEPS: SectionStep[] = [
  { key: 'approach',     label: 'Creative Approach', hint: 'How-we-edit approach entries shown on the creative page.', Form: ApproachForm,    api: creativeHouseApproachApi,    managePath: '/creative/approach' },
  { key: 'final-output', label: 'Project Media',     hint: 'Final output media (images / videos) for the project.',    Form: FinalOutputForm, api: creativeHouseFinalOutputApi, managePath: '/creative/final-output' },
];
...
const recordLabel = (rec: any) =>
  rec?.heading || rec?.title || rec?.name || rec?.slug || rec?._id || '';
...
export default function Wizard() {
  ...
  // /creative/wizard?itemId=<created item>&step=<n>
  ...
  useEffect(() => {
    ...
    creativeHouseItemApi.getOne(itemId)
      .then(({ data }) => setItemTitle(data.data?.title || data.data?.videoTitle || ''))
      .catch(() => setItemTitle(''));
  }, [itemId]);
  ...
  const refreshSection = useCallback((section: SectionStep) => {
    if (!itemId) return;
    section.api.getAll({ creativeHouseItemId: itemId, limit: 5 })
      .then(({ data }) => {
  ...
  const handleFinish = () => {
    toast.success('Creative item completed');
    navigate('/creative/item');
  };
  ...
  return (
    <div>
      <PageHeader
        title="Create Creative Item"
        breadcrumbs={[{ label: 'Creative House' }, { label: 'Create Item (Wizard)' }]}
        actions={itemId ? (
          ...
          {step === 0 ? (
            <div className="card w-full">
              ...
              <ItemForm
                ...
                onCancel={() => navigate('/creative/item')}
              />
            </div>
          ) : !itemId ? (
            ...
            <div className="card">
              <div className="mb-4 flex items-start justify-between gap-3">
                ...
                <Link
                  to={`${currentSection.managePath}?creativeHouseItemId=${itemId}`}
