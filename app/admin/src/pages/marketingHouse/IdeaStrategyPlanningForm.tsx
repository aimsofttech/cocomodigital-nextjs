...
export default function IdeaStrategyPlanningForm({ onSuccess, onCancel, editId, lockedItemId }: Props = {}) {
  ...
  useEffect(() => {
    if (isEdit && editId) {
      marketingHouseIdeaStrategyApi.getOne(editId).then(({ data }) => {
        const rec = data.data;
        reset({
          ...rec,
  ...
  const onSubmit = async (formData: any) => {
    ...
    ['title', 'description', 'image', 'slug', 'displayOrder', 'status'].forEach((k) => {
      if (formData[k] !== undefined && formData[k] !== '') fd.append(k, String(formData[k]));
    });
    try {
      if (isEdit && editId) await marketingHouseIdeaStrategyApi.update(editId, fd);
