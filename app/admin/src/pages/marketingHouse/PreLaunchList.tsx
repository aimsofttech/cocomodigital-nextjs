...
export default function PreLaunchList() {
  ...
  const columns = [
    ...
    { key: 'description', label: 'Description', sortable: true, render: (row: any) => <span className="text-xs text-gray-600 line-clamp-2 max-w-xs">{row.description || 'N/A'}</span> },
    { key: 'displayOrder', label: 'Order', sortable: true },
    { key: 'status', label: 'Status', sortable: true, render: (row: any) => <StatusToggle status={row.status} onConfirm={(newStatus) => handleStatusChange(row._id, newStatus)} /> },
  ];
  return (
    <CrudListPage title="Pre-Launch Activities" breadcrumbs={[{ label: 'Marketing Campaigns' }, { label: 'Pre-Launch' }]}
