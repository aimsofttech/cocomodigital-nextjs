...
import { marketingHouseOtherActivityCategoryApi, marketingHouseCategoryApi, marketingHouseItemApi } from '@/services/adminApi';
...
export default function OtherActivityCategoryModuleForm({ onSuccess, onCancel, editId, lockedItemId }: Props = {}) {
  ...
  useEffect(() => {
    if (isEdit && editId) {
      marketingHouseOtherActivityCategoryApi.getOne(editId).then(({ data }) => {
        const rec = data.data;
        reset({
          ...rec,
  ...
  // When locked to a specific item, force the field value and resolve its name.
  useEffect(() => {
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      if (isEdit && editId) await marketingHouseOtherActivityCategoryApi.update(editId, payload);
      else await marketingHouseOtherActivityCategoryApi.create(payload);
