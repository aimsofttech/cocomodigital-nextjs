...
import { marketingHouseOtherActivityItemApi, marketingHouseOtherActivityCategoryApi } from '@/services/adminApi';
...
export default function OtherActivityItemForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  useEffect(() => {
    marketingHouseOtherActivityCategoryApi.getAll({ limit: 100 }).then(({ data }) => setCategories(data.data || []));
    if (isEdit && id) {
      marketingHouseOtherActivityItemApi.getOne(id).then(({ data }) => {
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      if (isEdit && id) await marketingHouseOtherActivityItemApi.update(id, fd);
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div>
        ...
        <select {...register('marketingHouseOtherActivityCategoryId')} className="form-select">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div><label className="form-label">Video URL</label><input {...register('video_url')} className="form-input" placeholder="https://youtube.com/..." /></div>
      </div>
      <ImageUpload name="image" label="Item Image" recommended={{ width: 1200, height: 800, ratio: '3:2', formats: 'JPG, PNG, WebP', maxSizeMB: 2 }} uploadType="image" folder="marketing-house" value={watch('image')} onChange={(url) => setValue('image', url)} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Update Add-on Activities Item' : 'Add Add-on Activities Item'} breadcrumbs={[{ label: 'Add-on Activities Item', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
