...
export default function PreLaunchActivityList() {
  const { data, loading, submitting, pagination, remove, setSearch, setPage, setFilterParams, fetchAll } = useCrud(marketingHousePreLaunchApi);
  ...
  return (
    <CrudListPage title="PreLaunched Activity" breadcrumbs={[{ label: 'Marketing House' }, { label: 'Item Sections' }, { label: 'PreLaunched Activity' }]}
      columns={columns} data={data} loading={loading} submitting={submitting} pagination={pagination}
      onPageChange={setPage} onSearch={setSearch} onDelete={remove}
      filterFields={FILTER_FIELDS} onServerFilterChange={setFilterParams}
      renderModal={({ id, onSuccess, onCancel }) => <PreLaunchActivityForm editId={id} onSuccess={onSuccess} onCancel={onCancel} />}
