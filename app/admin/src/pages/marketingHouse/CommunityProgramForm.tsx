...
export default function CommunityProgramForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div>
        ...
        <input {...register('name', { required: 'Required' })} className="form-input" placeholder="Enter program name" />
        {errors.name && <p className="form-error">{String(errors.name.message)}</p>}
      </div>
      <SlugField register={register} watch={watch} setValue={setValue} isEdit={isEdit} />
      <ImageUpload name="image" label="Program Image" recommended={{ width: 600, height: 400, ratio: '3:2', formats: 'JPG, PNG, WebP', maxSizeMB: 2 }} uploadType="image" folder="marketing-house" value={watch('image')} onChange={(url) => setValue('image', url)} />
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Update Community Program' : 'Add Community Program'} breadcrumbs={[{ label: 'Community Program', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
