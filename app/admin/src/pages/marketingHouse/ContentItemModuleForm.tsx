...
export default function ContentItemModuleForm({ onSuccess, onCancel, editId }: Props = {}) {
    ...
    if (isEdit && editId) {
      marketingHouseContentItemApi.getOne(editId).then(({ data }) => {
        ...
        reset({
          ...
          content_created_category_id: rec.content_created_category_id ? String(rec.content_created_category_id) : '',
        });
  ...
  useEffect(() => {
    ...
    marketingHouseContentCategoryApi.getAll({ marketing_house_item_id: selectedItem, limit: 200 })
  ...
  const onSubmit = async (formData: any) => {
    ...
    ['content_created_category_id', 'item_title', 'item_video_url', 'item_image', 'slug', 'display_order', 'status'].forEach((k) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select
            {...categoryReg}
            onChange={(e) => { categoryReg.onChange(e); setValue('marketing_house_item_id', ''); setValue('content_created_category_id', ''); }}
        ...
        <div>
          ...
          <select
            {...itemReg}
            onChange={(e) => { itemReg.onChange(e); setValue('content_created_category_id', ''); }}
            className="form-select"
      ...
      <div>
        ...
        <select {...register('content_created_category_id')} className="form-select" disabled={!selectedItem}>
          <option value="">{selectedItem ? 'Select content category' : 'Select an item first'}</option>
          {contentCategories.map((c: any) => <option key={c._id} value={c._id}>{c.category_name || c.name}</option>)}
        </select>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
