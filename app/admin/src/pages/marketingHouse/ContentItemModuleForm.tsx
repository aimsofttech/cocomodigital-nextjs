...
export default function ContentItemModuleForm({ onSuccess, onCancel, editId, lockedItemId }: Props = {}) {
    ...
    if (isEdit && editId) {
      marketingHouseContentItemApi.getOne(editId).then(({ data }) => {
        ...
        reset({
          ...
          marketing_house_content_created_category_id: rec.marketing_house_content_created_category_id ? String(rec.marketing_house_content_created_category_id) : '',
        });
  ...
  const onSubmit = async (formData: any) => {
    ...
    ['marketing_house_content_created_category_id', 'image', 'display_order', 'status'].forEach((k) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      {!lockedItemId && (
        <div>
          ...
          <select
            {...categoryReg}
            onChange={(e) => { categoryReg.onChange(e); setValue('marketing_house_item_id', ''); setValue('marketing_house_content_created_category_id', ''); }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          {lockedItemId ? (
            ...
            <>
              <select
                {...itemReg}
                onChange={(e) => { itemReg.onChange(e); setValue('marketing_house_content_created_category_id', ''); }}
                className="form-select"
        ...
        <div>
          ...
          <select {...register('marketing_house_content_created_category_id')} className="form-select" disabled={!selectedItem}>
