...
export default function ContentItemForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <ImageUpload name="image" label="Item Image" recommended={{ width: 1200, height: 675, ratio: '16:9', formats: 'JPG, PNG, WebP', maxSizeMB: 2, note: 'keep one ratio across items so the grid stays even' }} uploadType="image" folder="marketing-house" value={watch('image')} onChange={(url) => setValue('image', url)} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Update Content Item' : 'Add Content Item'} breadcrumbs={[{ label: 'Content Item', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
