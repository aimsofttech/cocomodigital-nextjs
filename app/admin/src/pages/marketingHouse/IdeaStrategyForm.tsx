...
export default function IdeaStrategyForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  useEffect(() => {
    if (isEdit && id) {
      marketingHouseIdeaStrategyApi.getOne(id).then(({ data }) => {
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      if (isEdit && id) await marketingHouseIdeaStrategyApi.update(id, fd);
      else await marketingHouseIdeaStrategyApi.create(fd);
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div><label className="form-label">Description</label><textarea {...register('idea_description')} className="form-textarea" placeholder="Write a short description…" /></div>
