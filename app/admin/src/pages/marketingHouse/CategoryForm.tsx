...
export default function CategoryForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <ImageUpload name="marketing_house_icon" label="Icon" uploadType="image" folder="marketing-house" value={watch('marketing_house_icon')} onChange={(url) => setValue('marketing_house_icon', url)} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Edit Marketing Category' : 'Add Marketing Category'} breadcrumbs={[{ label: 'Marketing Category', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
