...
export default function CategoryForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  useEffect(() => {
    if (isEdit && id) {
      marketingHouseCategoryApi.getOne(id).then(({ data }) => {
  ...
  const onSubmit = async (data: any) => {
    try {
      if (isEdit && id) await marketingHouseCategoryApi.update(id, data);
      else await marketingHouseCategoryApi.create(data);
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <ImageUpload name="icon" label="Icon" recommended={{ width: 50, height: 50, ratio: '1:1', formats: 'JPG, PNG, JPEG, GIF', maxSizeMB: 1 }} uploadType="image" folder="marketing-house" value={watch('icon')} onChange={(url) => setValue('icon', url)} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Update Marketing Category' : 'Add Marketing Category'} breadcrumbs={[{ label: 'Marketing Category', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
