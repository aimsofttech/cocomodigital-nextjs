...
import { marketingHouseFaqApi, marketingHouseCategoryApi, marketingHouseItemApi } from '@/services/adminApi';
...
export default function MarketingFaqForm({ onSuccess, onCancel, editId, lockedItemId }: Props = {}) {
  ...
  useEffect(() => {
    if (isEdit && editId) {
      marketingHouseFaqApi.getOne(editId).then(({ data }) => {
        const rec = data.data;
        reset({
          ...rec,
  ...
  const onSubmit = async (formData: any) => {
    ...
    if (!formData.marketingHouseItemId) { toast.error('Please select an item'); return; }
    const payload = {
    ...
    try {
      if (isEdit && editId) await marketingHouseFaqApi.update(editId, payload);
      else await marketingHouseFaqApi.create(payload);
