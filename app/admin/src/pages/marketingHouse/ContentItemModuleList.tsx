...
export default function ContentItemModuleList() {
  ...
  const { data, loading, submitting, pagination, remove, setSearch, setPage, setFilterParams, fetchAll, setData } = useCrud(marketingHouseContentItemApi, true, itemId ? { marketingHouseItemId: itemId } : {});

  // Drag-and-drop rows to renumber displayOrder (shared hook).
  const handleReorder = useRowReorder({ api: marketingHouseContentItemApi, data, setData, pagination, fetchAll });
