...
export default function CommunityProgramItemForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <ImageUpload name="image" label="Item Image" recommended={{ width: 600, height: 400, ratio: '3:2', formats: 'JPG, PNG, WebP', maxSizeMB: 2 }} uploadType="image" folder="marketing-house" value={watch('image')} onChange={(url) => setValue('image', url)} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Update Community Program Item' : 'Add Community Program Item'} breadcrumbs={[{ label: 'Community Program Item', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
