    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input {...register('author_name', { required: 'Required' })} className="form-input" />
          {errors.author_name && <p className="form-error">{String(errors.author_name.message)}</p>}
        </div>
        <SlugField register={register} watch={watch} setValue={setValue} isEdit={isEdit} />
        <div><label className="form-label">Designation</label><input {...register('author_designation')} className="form-input" /></div>
      </div>
      <div><label className="form-label">Bio</label><textarea {...register('author_bio')} className="form-textarea" /></div>
