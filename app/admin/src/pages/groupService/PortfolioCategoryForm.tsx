...
export default function PortfolioCategoryForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const onSubmit = async (data: any) => {
    const payload = { ...data, ...(itemId ? { group_service_item_id: itemId } : {}) };
    try {
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div>
        ...
        <input {...register('portfolio_category_name', { required: 'Required' })} className="form-input" />
