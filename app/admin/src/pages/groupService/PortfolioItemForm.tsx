...
import { groupPortfolioItemApi, groupPortfolioCategoryApi } from '@/services/adminApi';
...
export default function PortfolioItemForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  useEffect(() => {
    groupPortfolioCategoryApi.getAll({ limit: 100 }).then(({ data }) => setCategories(data.data || []));
    if (isEdit && id) {
      groupPortfolioItemApi.getOne(id).then(({ data }) => {
        const item = data.data;
        reset({ ...item, status: String(item.status), portfolio_category_id: item.portfolio_category_id?._id || item.portfolio_category_id });
  ...
  const onSubmit = async (formData: any) => {
    try {
      if (isEdit && id) await groupPortfolioItemApi.update(id, formData);
      else await groupPortfolioItemApi.create(formData);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div>
        ...
        <select {...register('portfolio_category_id', { required: 'Required' })} className="form-select">
          ...
          {categories.map((c: any) => <option key={c._id} value={c._id}>{c.portfolio_category_name}</option>)}
        </select>
        {errors.portfolio_category_id && <p className="form-error">{String(errors.portfolio_category_id.message)}</p>}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div><label className="form-label">Item Title</label><input {...register('portfolio_item_title')} className="form-input" /></div>
        <div><label className="form-label">Video URL</label><input {...register('portfolio_item_video_url')} className="form-input" placeholder="https://youtube.com/..." /></div>
      </div>
