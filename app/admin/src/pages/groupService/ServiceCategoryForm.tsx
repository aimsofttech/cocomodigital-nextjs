...
export default function ServiceCategoryForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  const onSubmit = async (data: any) => {
    try {
      if (isEdit && id) await groupServiceCategoryApi.update(id, data);
      else await groupServiceCategoryApi.create(data);
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div>
        ...
        <input {...register('group_service_category_name', { required: 'Required' })} className="form-input" />
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Edit Group Service Category' : 'Add Group Service Category'} breadcrumbs={[{ label: 'Group Service Category', path: '../' }, { label: isEdit ? 'Edit' : 'Add' }]} />
