...
export default function SuccessStoriesList() {
  const { data, loading, submitting, pagination, remove, setSearch, setPage, setFilterParams, fetchAll } = useCrud(successStoriesApi);
  ...
  const columns = [
    ...
    { key: 'success_stories_url', label: 'URL', sortable: true, render: (row: any) => row.success_stories_url ? <a href={row.success_stories_url} target="_blank" rel="noreferrer" className="text-primary-600 hover:underline text-xs">{row.success_stories_url}</a> : '—' },
