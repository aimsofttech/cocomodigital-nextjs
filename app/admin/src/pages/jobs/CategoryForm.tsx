...
export default function CategoryForm({ onSuccess, onCancel, editId }: Props = {}) {
  ...
  useEffect(() => {
    if (isEdit && id) {
      jobCategoryApi.getOne(id).then(({ data }) => {
  ...
  const onSubmit = async (data: any) => {
    try {
      if (isEdit && id) await jobCategoryApi.update(id, data);
      else await jobCategoryApi.create(data);
      toast.success(isEdit ? 'Updated' : 'Created');
      if (onSuccess) onSuccess(); else navigate('/jobs/category');
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="flex gap-3 pt-2">
        <button type="button" onClick={() => onCancel ? onCancel() : navigate('/jobs/category')} className="btn-secondary flex-1">Cancel</button>
  ...
  return (
    <div>
      <PageHeader title={isEdit ? 'Edit Job Category' : 'Add Job Category'}
        breadcrumbs={[{ label: 'Job Category', path: '/jobs/category' }, { label: isEdit ? 'Edit' : 'Add' }]} />
