...
/**
 * One-time migration: rename Video (home "Growth at a glance" video)
 * document keys to camelCase (dropping the redundant `video_` prefix) and
 * backfill mongoose-standard createdAt/updatedAt from the legacy Laravel
 * created_at/updated_at fields. Mirrors rename-brand-keys.js.
 *
 * Run:   node scripts/rename-video-keys.js     (from app/api)
 *
...
(async () => {
  ...
  const col = mongoose.connection.db.collection('video');

  const renameRes = await col.updateMany({}, { $rename: RENAMES });
  console.log(`video renames: matched ${renameRes.matchedCount}, modified ${renameRes.modifiedCount}`);
  ...
  console.log(`video timestamps: ${docs.length} docs, ${migrated} migrated`);
