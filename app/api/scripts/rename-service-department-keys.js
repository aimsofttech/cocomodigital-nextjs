...
/**
 * One-time migration: rename ServiceCategory ("Service Department") document
 * keys to camelCase (dropping the legacy `service_` prefix) and backfill
 * mongoose-standard createdAt/updatedAt from the legacy Laravel
 * created_at/updated_at fields. Mirrors rename-brand-keys.js.
 *
 * Run:   node scripts/rename-service-department-keys.js     (from app/api)
 *
...
(async () => {
  ...
  console.log(`explore_our_service_category renames: matched ${renameRes.matchedCount}, modified ${renameRes.modifiedCount}`);
  ...
  console.log(`explore_our_service_category timestamps: ${docs.length} docs, ${migrated} migrated`);
