...
/**
 * One-time migration: rename MarketingHouseCategory ("Marketing Campaigns →
 ...
 *
 * Run:   node scripts/rename-marketing-category-keys.js     (from app/api)
 *
...
(async () => {
  ...
  const col = mongoose.connection.db.collection('marketing_house_categories');

  const renameRes = await col.updateMany({}, { $rename: RENAMES });
  console.log(`marketing_house_categories renames: matched ${renameRes.matchedCount}, modified ${renameRes.modifiedCount}`);
  ...
  console.log(`marketing_house_categories timestamps: ${docs.length} docs, ${migrated} migrated`);
